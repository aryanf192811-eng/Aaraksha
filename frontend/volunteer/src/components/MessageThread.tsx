...
// extends the tel: call button already on ActiveJobPage.tsx.
...
const SENDER_LABELS: Record<Message['sender_kind'], string> = {
  TOURIST: 'Tourist', GUARDIAN: 'Guardian', VOLUNTEER: 'You', TEAM: 'You',
}
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2.5 min-h-0">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center gap-2 text-on-surface-variant px-6">
            ...
            <p className="text-xs">No messages yet — let them know anything useful before you arrive.</p>
          </div>
        ) : (
          messages.map((m) => {
            const isMine = m.sender_kind === 'VOLUNTEER' || m.sender_kind === 'TEAM'
            return (
