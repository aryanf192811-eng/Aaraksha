...
// No drag/gesture library exists anywhere in this codebase (only precedent:
// SOSButton.tsx's onPointerDown/onPointerUp hold-to-confirm gesture) — this
// is a small, dependency-free pointer-based drag-to-dismiss, built on
// native Pointer Events rather than pulling in a new package for one
// gesture. Attach `handleProps` to a drag-handle element only (not the
// whole sheet), so dragging never fights the message list's own scroll.
