...
export function connectSocket(role: 'tourist' | 'govt' | 'guardian', tokenOrGuardianToken: string): Socket {
  const identity = `${role}:${tokenOrGuardianToken}`
  if (_socket?.connected && _identity === identity) return _socket
  ...
  // logout+login would keep broadcasting into the PREVIOUS user's room
