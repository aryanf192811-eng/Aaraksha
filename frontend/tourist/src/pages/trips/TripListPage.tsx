...
export default function TripListPage() {
  ...
  const { mutate: joinTrip, isPending: joining } = useMutation({
    ...
    onSuccess: (res) => {
      toast.success('Joined the group trip!')
    ...
    onError: (err: any) => {
      toast.error(err?.response?.data?.message || 'Could not join — check the invite code')
    },
  })
  ...
  return (
    <div className="min-h-screen bg-surface pb-24">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <Button onClick={() => setJoinOpen(true)} size="sm" variant="outline"
              ...
              <Users className="w-3.5 h-3.5 mr-1" /> Join
            </Button>
      ...
      <Dialog open={joinOpen} onOpenChange={setJoinOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Join a group trip</DialogTitle>
            <DialogDescription>Enter the invite code a travel companion shared with you.</DialogDescription>
          </DialogHeader>
          <Input
            value={joinCode}
