    ...
    <div className="min-h-screen bg-surface pb-24">
      ...
      <form onSubmit={onSubmit} className="px-5 mt-5 space-y-4">
        ...
        <div className="bg-surface-container-lowest rounded-2xl shadow-sm p-5 space-y-4">
          <h3 className="font-bold text-on-surface">Basic Info</h3>
        ...
        {/* Health info */}
        <div className="bg-surface-container-lowest rounded-2xl shadow-sm p-5 space-y-4">
          <h3 className="font-bold text-on-surface">Health Information</h3>
          <div className="space-y-1.5">
            ...
            >
              <SelectTrigger className="h-12 rounded-xl"><SelectValue placeholder="Select blood group" /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-on-surface-variant">Shown to rescuers and your Guardian contacts in an emergency.</p>
          </div>
        </div>

        {/* Emergency contacts */}
        <div className="bg-surface-container-lowest rounded-2xl shadow-sm p-5 space-y-3">
          <h3 className="font-bold text-on-surface">Emergency Contacts</h3>
          {fields.map((field, idx) => (
            <div key={field.id} className="bg-surface-container rounded-xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Input placeholder="Name" className="h-10 rounded-lg text-sm" {...form.register(`emergencyContacts.${idx}.name`)} />
                <Input placeholder="10-digit mobile" type="tel" className="h-10 rounded-lg text-sm" {...form.register(`emergencyContacts.${idx}.phone`)} />
                <Input placeholder="Relation (Parent, Friend...)" className="h-10 rounded-lg text-sm col-span-2" {...form.register(`emergencyContacts.${idx}.relation`)} />
              </div>
