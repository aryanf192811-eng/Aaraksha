    ...
    <div className="fixed inset-0 z-[1100] flex items-end sm:items-center sm:justify-center bg-black/40" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={sheetStyle}
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-4 py-4 space-y-4">
          {/* Badges — same connectivity/difficulty/zone vocabulary the stop card uses */}
          <div className="flex flex-wrap gap-1.5">
            <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-surface-container text-on-surface-variant">{tEnum(t, 'zoneType', stop.zone_type)}</span>
            <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-surface-container text-on-surface-variant">{tEnum(t, 'connectivity', stop.connectivity)}</span>
            <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-surface-container text-on-surface-variant">{tEnum(t, 'difficulty', stop.difficulty)}</span>
            {stop.altitude_m > 2000 && (
              <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-tsi-high/10 text-tsi-high flex items-center gap-1">
                <Mountain className="w-3 h-3" /> {stop.altitude_m}m
              </span>
            )}
          </div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 gap-2">
                ...
                {destination.ilp_required && (
                  <div className="flex items-center gap-2.5 text-xs">
                    <span className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0"><Shield className="w-4 h-4 text-primary-dark" /></span>
                    <span className="text-on-surface-variant">Inner Line Permit (ILP) required for this destination</span>
                  </div>
                )}
              </div>
            </>
